...
export class EncryptionHelper {
  ...
  static async decrypt(value: string): Promise<string> {
    const decryptedValue = await EncryptionUtil.decryptWithAESGCM(
      urlEncryptionKey,
      value
    );
  ...
  static async encryptObjectFields(
    ...
    for (const field of fields) {
      if (encryptedObj[field]) {
        encryptedObj[field] = await EncryptionHelper.encrypt(
          encryptedObj[field]
        );
      }
    }
  ...
  static async decryptObjectFields(
    ...
    for (const field of fields) {
      if (decryptedObj[field]) {
        decryptedObj[field] = await EncryptionHelper.decrypt(
          decryptedObj[field]
        );
      }
    }
  ...
  static async encryptURL(url: URL, fields: string[]): Promise<URL> {
    const encryptedParams = await EncryptionHelper.encryptURLSearchParams(
      url.searchParams,
      fields
    );
    const encryptedURL = new URL(
      `${url.origin + url.pathname}?${encryptedParams.toString()}`
    );
    return encryptedURL;
  }

  static async decryptURL(url: URL, fields: string[]): Promise<URL> {
    const decryptedParams = await EncryptionHelper.decryptURLSearchParams(
      url.searchParams,
      fields
    );
    const decryptedURL = new URL(
      `${url.origin + url.pathname}?${decryptedParams.toString()}`
    );
    return decryptedURL;
  }
  ...
  static async decryptURLSearchParams(
    ...
    for (const key of params.keys()) {
      ...
      if (key === "subscription_key") {
        params.set(key, decodeURIComponent(params.get(key) || ""));
      }
