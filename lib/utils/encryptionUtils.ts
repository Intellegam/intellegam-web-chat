class EncryptionUtil {
	static async encryptWithAESGCM(key: string, data: string): Promise<string> {
		...
		const encrypted = await crypto.subtle.encrypt(algorithm, keyMaterial, encodedData);
	...
	static async decryptWithAESGCM(
		...
		try {
			const data = Uint8Array.from(atob(encryptedDataWithIv), (c) => c.charCodeAt(0));
			...
			const algorithm = { name: 'AES-GCM', iv };
			const decrypted = await crypto.subtle.decrypt(algorithm, keyMaterial, encryptedData);
	...
	static async generateHMACSHA256Signature(key: string, message: string): Promise<string> {
		const encoder = new TextEncoder();
		const encodedKey = encoder.encode(key);
