...
export async function deleteUser(id: string) {
  try {
    return await getDB().delete(user).where(eq(user.id, id));
  } catch (error) {
...
export async function deleteUserByWorkOSId(id: string) {
  try {
    return await getDB().delete(user).where(eq(user.workosId, id));
  } catch (error) {
...
export async function createGuestUser() {
  ...
  try {
    return await getDB().insert(user).values({ email }).returning({
      id: user.id,
      email: user.email,
    });
  } catch (error) {
